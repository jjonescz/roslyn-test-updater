...
This command-line tool automatically updates snapshot/baseline tests (currently handles only `VerifyDiagnostics`) in the [Roslyn repository](https://github.com/dotnet/roslyn).

## Building

```ps1
dotnet publish src/RoslynTestUpdater/RoslynTestUpdater.csproj -c Release /p:PublishProfile=FolderProfile
```
...
The tool automatically patches "expected" test code to match "actual".
...
If the `--write-playlist` option is specified on the command-line, the tool also writes `test.playlist` into the current directory with a list of failed tests (in Visual Studio Test Explorer format).
...
1. Copy Tests Output (<kbd>Ctrl+A</kbd>, <kbd>Ctrl+C</kbd>):
